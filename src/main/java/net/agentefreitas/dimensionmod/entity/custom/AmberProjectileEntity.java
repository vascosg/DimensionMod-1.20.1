...
public class AmberProjectileEntity extends Projectile {

    // Construtor para o Registro (ModEntities)
    public AmberProjectileEntity(EntityType<? extends AmberProjectileEntity> pEntityType, Level pLevel) {
    ...
    public AmberProjectileEntity(Level level, LivingEntity shooter) {
        this(ModEntities.AMBER_PROJECTILE.get(), level);
    ...
    protected void onHitEntity(EntityHitResult result) {
        if (!this.level().isClientSide) {
            ...
            if (target != owner) {
                target.hurt(this.damageSources().inFire(), 12.0F);
